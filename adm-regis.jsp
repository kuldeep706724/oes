...
function demo(){
	...
	if(uname()&upass()&con()&mail()&city()&passcode()){
		return true;
	}
	else{
		return false;
...
function city(){
	a=document.getElementById("t7").value;
	if(a.length>=1){
		return true;
	}
	else{
		document.getElementById("s7").innerHTML="Enter valid city";
		return false;
	}
}
function passcode(){
	...
	if(a=="1234"){
		...
		document.getElementById("s8").innerHTML="Enter correct passcode";
		return false;
	}
}
	...
	<h1>Admin Register </h1>
            <form action="adm-regis.jsp" name="abc" method="post">
            <p>Username</p>
            <input type="text" name="username" placeholder="Enter Username" required id="t1">
			<span id="s1" style="color:red"></span>
            <p>Name</p>
            <input type="text" name="name" placeholder="Enter Name" required id="t2">
			<span id="s2" style="color:red"></span>
            <p>Password</p>
            <input type="password" name="password" placeholder="Enter Password" required id="t3">
			<span id="s3" style="color:red"></span>
			<p>Contact</p>
            <input type="text" name="contact" placeholder="Enter Contact" required id="t4">
			<span id="s4" style="color:red"></span>
			<p>Email</p>
            <input type="text" name="email" placeholder="Enter Email" required id="t5">
			<span id="s5" style="color:red"></span>
			<p>Date of Birth</p>
            <input type="date" name="dob" placeholder="Enter DOB" required id="t6">
			<span id="s6" style="color:red"></span>
			<p>City</p>
            <input type="text" name="city" placeholder="Enter City" required id="t7">
			<span id="s7" style="color:red"></span>
			<p>Gender</p><br>
            <input type="radio" name="a" value="MALE" required="required" id="a"> MALE
			<input type="radio" name="a" value="FEMALE" required="required" id="a">FEMALE			
			<!--<p>Passcode (contact owner for passcode)</p>
            <input type="text" name="city" placeholder="Enter Passcode" id="t8">
...
String s2=request.getParameter("username");
	...
	try{
		...
		String q="select * from admregis where Username='"+s2+"'";
		...
		String q1="insert into admregis values('"+s2+"','"+s3+"','"+s4+"','"+s5+"','"+s6+"','"+s8+"','"+s9+"','"+s7+"')";
		st.executeUpdate(q1);
		%>
		<script type="text/javascript">
